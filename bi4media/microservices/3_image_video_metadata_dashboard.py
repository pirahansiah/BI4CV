...
def create_dash_app():
    ...
    app.layout = html.Div([
        html.H1("Image Metadata Dashboard", style={'textAlign': 'center', 'color': '#007BFF'}),
        html.Div([
            html.P(f"Folder Path: {folder_path}", style={'fontWeight': 'bold'}),
            html.P(f"Total Folders: {len(next(os.walk(folder_path))[1])+1}", style={'fontWeight': 'bold'}),
            html.P(f"Total Files: {sum(info['images'] + info['videos'] for info in folder_info.values())}", style={'fontWeight': 'bold'})
        ...
        html.Div([
            ...
            ], style={'width': '80%', 'margin': 'auto', 'border': '1px solid #ddd', 'borderCollapse': 'collapse'}),
        ]),
        dcc.Loading(
            id="loading",
            type="default",
        ...
        if metadata_df.empty:
            ...
            return dcc.Tabs([
                dcc.Tab(label='File Count by Format', children=[
                    dcc.Graph(figure=px.bar(metadata_df, x='format', title='File Count by Format', labels={'x': 'Format', 'y': 'Count'}, color='format'))
                ]),
                dcc.Tab(label='File Size Distribution', children=[
                    dcc.Graph(figure=px.histogram(metadata_df, x='file_size', title='File Size Distribution', labels={'x': 'File Size', 'y': 'Count'}, nbins=50))
                ]),
                dcc.Tab(label='Resolution Distribution', children=[
                    dcc.Graph(figure=px.histogram(metadata_df[metadata_df['resolution'].notna()], x='resolution', title='Resolution Distribution', labels={'x': 'Resolution', 'y': 'Count'}, nbins=50))
                ]),
                dcc.Tab(label='Creation Date Timeline', children=[
                    dcc.Graph(figure=px.line(metadata_df.sort_values(by='created_at'), x='created_at', y='file_size', title='File Size Over Time', labels={'x': 'Created At', 'y': 'File Size'}))
                ]),
                dcc.Tab(label='Image Quality Details', children=[
                    ...
                    ], style={'width': '100%', 'margin': 'auto', 'border': '1px solid #ddd', 'borderCollapse': 'collapse'}),
                ])
